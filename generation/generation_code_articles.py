...
def generate_prompts(instruction, data, randomize=False, context_num=1, model_name = '', sys_prompt=''):
    prompts = []
...
retrieved_data = [(el, pd.read_json(folder_path+el).T) for el in os.listdir(folder_path) if "articles" in el]
...
for file_name, configuration in retrieved_data:
    ...
    df.to_json(f"results/no_repetition/articles/1_{final_file_name}", indent=4)
